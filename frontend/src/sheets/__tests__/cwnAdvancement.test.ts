...
import { xpForLevel, xpProgress, describeXp, CWN_MAX_LEVEL, CWN_XP_RATES } from '../cwnAdvancement';
...
const at = (over = {}) => ({ level: 1, xp: 0, xp_rate: 'fast', ...over });
...
describe('the thresholds the book prints', () => {
  ...
  it('offers exactly the two rates the book prints', () => {
    expect(CWN_XP_RATES.map((r) => r.value)).toEqual(['fast', 'slow']);
  });
});

describe('where a character is between levels', () => {
  it('names the next level and what it needs', () => {
    const p = xpProgress(at({ level: 3, xp: 8 }));
  ...
  it('measures the bar from this level, not from zero', () => {
    ...
    expect(xpProgress(at({ level: 3, xp: 9 })).fraction).toBeCloseTo(0.5);
    expect(xpProgress(at({ level: 3, xp: 6 })).fraction).toBe(0);
  });

  it('says ready rather than levelling anyone up', () => {
    // Advancing grants skill points and sometimes a Focus. Both are choices.
    const p = xpProgress(at({ level: 3, xp: 12 }));
  ...
  it('follows the slow column when the table picked it', () => {
    const p = xpProgress(at({ level: 3, xp: 12, xp_rate: 'slow' }));
    expect(p.nextAt).toBe(24);
    expect(p.ready).toBe(false);
  });

  it('stops at the top of the table', () => {
    const p = xpProgress(at({ level: 10, xp: 400 }));
...
describe('sheets that are not filled in yet', () => {
  it('reads a blank level as 1, which is where the book starts operators', () => {
    expect(xpProgress({}).level).toBe(1);
    expect(xpProgress({ level: 0 }).level).toBe(1);
    expect(xpProgress(null).level).toBe(1);
  });
  ...
  it('defaults to the fast column rather than refusing to draw', () => {
    ...
    expect(xpProgress({ level: 2, xp_rate: 'nonsense' }).nextAt).toBe(6);
  });

  it('never reads negative XP or a level past the cap', () => {
    expect(xpProgress(at({ xp: -50 })).xp).toBe(0);
    expect(xpProgress(at({ level: 99 })).level).toBe(10);
  });
});
...
describe('where the fields live', () => {
  ...
  it('wires the bar to the fields it reads', () => {
    expect(CWN.header?.xpBar).toEqual({ xpField: 'xp', levelField: 'level', rateField: 'xp_rate' });
  });
  ...
  it('is on no other system', () => {
    for (const id of ['cyberpunk_red', 'shadowrun_6e', 'generic']) {
      ...
      expect(ids, id).not.toContain('xp_rate');
    }
  });
});
