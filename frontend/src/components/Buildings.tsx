...
export const generateThemedBuildingsForPlot = (
  ...
  if (zoneTypeVal >= 1.5 && zoneTypeVal < 2.0) {
    ...
    if (landmarkStyle === 0) {
      ...
      const crystalW = Math.min(baseW, baseD) * 0.35;
      const root = { name: '', description: '', x: bx, y: 0, z: bz, width: crystalW * 1.2, depth: crystalW * 1.2, height: h * 0.1, color, shape: 'box', polyCount: 5 };
      rawBuildings.push(root);
      const key = getGridKey(bx, bz); if(!spatialGrid[key]) spatialGrid[key] = []; spatialGrid[key].push(root);
      ...
      for (let i = 0; i < facets; i++) {
        ...
        const facetX = bx + Math.cos(angle) * crystalW * 0.4;
        const facetZ = bz + Math.sin(angle) * crystalW * 0.4;
        const facetH = h * (0.3 + (i % 3) * 0.25);
        rawBuildings.push({ name: '', x: facetX, y: h * 0.1, z: facetZ, width: crystalW * 0.5, depth: crystalW * 0.5, height: facetH, color, shape: 'pyramid', polyCount: 5, parent_name: 'ROOT' });
      }

      // Central spire
      rawBuildings.push({ name: '', x: bx, y: h * 0.4, z: bz, width: crystalW * 0.4, depth: crystalW * 0.4, height: h * 0.55, color: '#00ffff', shape: 'pyramid', polyCount: 5, parent_name: 'ROOT' });
    }
    return;
  }
