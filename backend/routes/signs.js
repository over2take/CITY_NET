...
module.exports = (db, io, { emitUpdate, recordAction }) => {
  ...
  router.post('/', authenticate, (req, res) => {
    const { text, x, y, z, rotation_y, font_size, image_url, use_tv_filter } = req.body;
    if (!text || x == null || y == null || z == null) {
      return res.status(400).json({ error: 'text, x, y, z are required' });
    }
    // Sanitise: cap text length, reject javascript: and data: image URLs
    ...
    db.run(
      `INSERT INTO signs (text, x, y, z, rotation_y, font_size, image_url, use_tv_filter) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [safeText, x, y, z, rotation_y ?? 0, font_size ?? 1.0, safeImageUrl, use_tv_filter ? 1 : 0],
      function (err) {
        if (err) return res.status(500).json({ error: err.message });
