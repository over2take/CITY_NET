...
│   ├── buildingTypes.js       # What a building is for, and which types trade. Distinct from `classification`, which is the mesh a custom structure is drawn from - a ripperdoc and a noodle bar can share a shape
│   ├── net/
...
│   │   │   ├── ShopWindow.tsx          # A building that trades: what it carries, and a way to take a piece away. BUY lands an unplaced row on your sheet and stops - no money moves, no stock is kept - because buying is a transaction and installing is surgery. Buying and selling are tabs rather than two buttons on a row, since selling reads what you are carrying rather than what the shop stocks
...
│   │   ├── data/
│   │   │   └── buildingTypes.ts # What a building is for, mirrored from the server, plus which systems have shops at all. Cities Without Number only for now, and the gate is enforced on the server too - a button nobody can see is a courtesy, refusing the request is the rule
│   │   ├── sheets/
