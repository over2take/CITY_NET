...
│   │   │   ├── BuildingPreview.tsx     # The corner picture: the GM's photo, else the building's own parts turning as a wireframe in its own small canvas, else the CITY_NET badge. The colour is read from the theme's CSS variable where it sits - the info windows render outside the ThemeContext provider, so the context alone drew every theme in classic green
...
│   │   │   ├── CityNetIcon.tsx         # The CITY_NET badge from assets/citynet-logo.svg in the theme's colours, for anywhere that needs a picture and has none
...
│   │       ├── buildingParts.ts    # A building's root and child parts in the root's own frame, centred and measured, so a preview can draw the building alone and fit it to a box - the same arithmetic Buildings.tsx places them with
