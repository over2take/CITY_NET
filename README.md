...
│   │   ├── cyberwareLocations.ts # The nine install types, three of them paired, and where each lands on the figure. Side is a property of a row rather than part of the type, so sorting a list by type does not split somebody's two arms apart. The anchors were measured by hit-testing the drawing rather than guessed — and the figure's centreline is 0.428, not 0.5
│   │   ├── cyberwareRows.ts    # The row shape, mirrored from the backend. Humanity loss and eddies are different costs and both are kept; an unpriced piece stays blank rather than defaulting to zero, or it sorts as the cheapest thing on the sheet
