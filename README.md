...
│   │   ├── owned.js            # What a character owns, derived from the four places a sheet keeps things. Never cached - a manifest would go stale on every path that removes an item
│   │   ├── sell.js             # Prices a sell basket and builds the sheet patch. A weapon slot is cleared in full, a vehicle takes its mounts with it, and a basket that fails part way empties nothing
...
│   │   ├── catalogueStore.js   # Uploaded catalogues in memory, added on top of the built-in ones, never replacing them. Requires nothing, so priceOf stays synchronous and the lot stays importable from a frontend test
...
│   │   ├── data/
│   │   │   ├── buildingTypes.ts # What a building is for and which catalogues it sells, mirrored from the server, plus which systems have shops at all. Cities Without Number only for now, deliberately - buying and selling still assume CWN's sheet, and the note at the gate says what has to change before it widens
...
│   │   ├── ownedItems.ts       # What a character owns, from inventory rows, weapon slots and stash, cyberware and vehicle slots. Mirrored from backend/shops/owned.js and run against it over sixteen awkward sheets. Boxed chrome is listed before installed, so selling one of two sells the spare
