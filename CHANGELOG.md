...
- **Mounts can be fired.** They appear in the weapon picker after your carried weapons, labelled with their vehicle, and resolve through the server path that was already there. Rows are keyed by the *(vehicle, mount)* pair rather than by index, since mount 1 on two different cars are both weapon 1 and one would otherwise quietly shadow the other. The counts come from the sheet template rather than being retyped, because a client that lists fewer vehicles than the sheet declares strands the last ones' guns with nothing to show for it.
- **A gunner can fire the guns of a car they are riding in.** Firing a mount costs the gunner their main action, which assumes a gunner who is not necessarily the owner — but mounts resolved off your own sheet, so the only person who could use a turret was whoever owned it. The resolution needed no changes: the to-hit was already built from the attacker's sheet, so a gunner fires someone else's cannon with their own skill and attribute mod, and only the weapon row moves. Since a passenger cannot see the sheet those rows live on, the server sends them the mounts of the car they are in — the mount rows only, and only to someone who has declared they are sitting in it.

  There is no seat check and no limit on how often a mount fires. Nothing else in the combat system polices action economy, and mounts are not the place to start.
- **Attacks on someone in a vehicle hit the vehicle.** Armour Rating is subtracted from the damage and the remainder comes off the vehicle's HP; the people inside are untouched until it is destroyed, at which point it stops being cover and the next shot resolves against them normally. A vehicle's AC is its own plus the driver's Drive skill while moving, and four *worse* while stationary; firing out of a moving one costs four either way, mount or carried gun.

  Damage lands on the owner's sheet, so four people in one car share one pool rather than each taking the same hit separately. This is what makes riding along worth declaring: `RIDING IN` points at another player's vehicle by name, and everything about it — HP, armour, AC — is read from their sheet.

  Every unreadable state resolves to *on foot*: an owner who logged out or was purged, a vehicle row never filled in, a wreck. That direction is deliberate. A broken reference should cost someone their cover, never make them impossible to hit.
- **The vehicle you are in is visible before someone shoots at it.** The derived combat numbers are mirrored onto the token the same way the character's name and description already were — the numbers you are shooting at, never the sheet itself. Attackers see the vehicle's name, AC, Armour Rating and HP in the attack panel before they commit, and the token carries a badge that is always drawn rather than revealed on hover, because cover you have to hover to discover is cover nobody accounts for. The roll history names the vehicle and shows the subtraction, so a hit mostly eaten by armour reads as armour rather than as a bug.

  Passengers are re-mirrored when the owner saves, since their badge is derived from someone else's sheet and would otherwise go quietly stale.
...
- **Sheet dropdowns can name their choices from the sheet.** A select's options may now be derived from the sheet's own data rather than declared up front. `RIDING IN` uses it to list your vehicles by name — you know you drive the Kestrel, not that the Kestrel is vehicle 2 — and to offer only the ones you have actually filled in. A template that names its own blank state ("ON FOOT") no longer gets a second, empty placeholder above it.
- **You pick who you are riding with from a list instead of typing their name.** It needs a *login* name, and a typo used to drop you back to being on foot — no cover, and nothing on screen to say why. The field is now a dropdown of the other players holding a sheet in this system, which is the right list rather than whoever is online: the car is read off their sheet, and that exists whether or not they are connected. It is called `RIDING WITH`, which is what it means — `WHOSE` never said whose, and `OWNER` claimed something about the vehicle rather than describing the arrangement.
- **The badge names everyone in the car.** Who is already aboard is the thing you actually ask at the table, and three people behind one Armour Rating is a different decision from one. The list appears on the token and in the attack panel, and only when the car is shared — your own name back at you is noise.

  There is no seat count. Recording a capacity would only matter if something enforced it, and the software does not overrule the table on how many people fit in a car any more than it counts your actions.

  The mirror now recomputes every player rather than the one who saved. Boarding someone's car changes what *their* badge should say as much as yours, and working out which subset to refresh means working out the whole thing anyway — on a table with one row per player.
- **A CWN attack now reads the defender's sheet before rolling to hit, not after.** Whether they are in a vehicle decides the AC, so it can no longer be fetched lazily at damage time. It was already being fetched twice further down, so this is one read where there were two.
- **New column on `locations`: `vehicle_state`.** Added by the usual startup migration — no action needed on upgrade, and it is empty for everyone not sitting in a car.
